  ...
  const validation = async (e) => {
    ...
    if (name === 'username') {
      ...
      dispatch({
        ...
        errMsg: {
          username: `${
            validUsername ? '' : '帳號名稱需至少8位數，且不得包含特殊符號'
          }`,
        },
      })
    ...
    if (name === 'password') {
      ...
      dispatch({
        ...
        errMsg: {
          password: `${
            validPwd ? '' : '密碼需至少8位數，且需包含大小寫及特殊符號'
          }`,
        },
      })
    }
  ...
  return (
    <form>
      <FlexColBox sx={{ justifyContent: 'start' }} marginY={5}>
        {step === 1 && (
          <>
            <AuthHeader subtitle="可以填寫真名也可以保持神秘。">
              你的名字是？
            </AuthHeader>
            <StyledTextField
              autoComplete="false"
              ...
              onChange={(e) =>
                dispatch({ type: 'fullnameInput', fullname: e.target.value })
              }
            />
          </>
        )}
        ...
        {step === 4 && (
          <>
            <AuthHeader subtitle="飲食健康有助於密碼記憶">
              請輸入密碼
            </AuthHeader>
            <StyledTextField
              autoComplete="false"
        ...
        {step === 5 && (
          <>
            ...
            <StyledTextField
              ...
              onChange={(e) =>
                dispatch({ type: 'birthInput', birth_date: e.target.value })
              }
            />
          </>
        )}
        ...
        {step < 5 && (
          <ArrowButton type="button" onClick={handleNextStep}></ArrowButton>
        )}
        {step < 5 && step > 1 && (
          <Button
            type="button"
        ...
        {step === 5 && (
          <ArrowButton type="submit" onClick={(e) => onSubmit(e)}></ArrowButton>
        )}
      </FlexColBox>
    </form>
  )
