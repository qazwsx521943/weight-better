...
import { Box, Container, TextField, Stack, Typography, Alert } from "@mui/material";
...
function UpdatePassword() {
    ...
    const handleInputChange = (e) => {
        dispatch({ type: "inputChange", passwordValue: { [e.target.name]: e.target.value }, error: { [e.target.name]: "" } });
    };

    const { currentPassword, changePassword, confirmChangePassword } = state.passwordValue;

    const updatePassword = async () => {
        dispatch({ type: "submitChange", saving: true });
        // 前端驗證
        if (currentPassword === "" || changePassword === "" || confirmChangePassword === "") {
        ...
        if (response.data.error) {
            dispatch({ type: "responseError", error: response.data.error, saving: false });
            return;
        } else {
            setTimeout(() => {
                dispatch({ type: "responseSuccess", success: response.data.success, saving: false });
            }, 1500);
        }
    };
    const isButtonDisabled = !(currentPassword !== "" && changePassword !== "" && confirmChangePassword !== "");
    ...
    return (
        <Box>
            <FlexColBox>
                ...
                <PrimaryButton variant="outlined" onClick={updatePassword} disabled={isButtonDisabled}>
